...
func SJFSchedule(w io.Writer, title string, processes []Process) {
	...
	for i := range processes {
		...
		if processes[i].ArrivalTime> turnaround{
			waitingTime = 0;
			lastCompletion = float64(completion)-2
		}
...
func SJFPrioritySchedule(w io.Writer, title string, processes []Process) {
	...
	for len(remainingProcesses) > 0 {
		...
		nextProcess := remainingProcesses[0]
		remainingProcesses = remainingProcesses[1:]

		if nextProcess.ArrivalTime > 0 {
			waitingTime = serviceTime - nextProcess.ArrivalTime
		}
		totalWait += float64(waitingTime)

		start := waitingTime + nextProcess.ArrivalTime

		turnaround := nextProcess.BurstDuration + waitingTime
		totalTurnaround += float64(turnaround)

		completion := nextProcess.BurstDuration + nextProcess.ArrivalTime + waitingTime
		lastCompletion = float64(completion)

		if nextProcess.ArrivalTime > turnaround {
			waitingTime = 0
			lastCompletion = float64(completion) - 2
		}
		...
		serviceTime += nextProcess.BurstDuration
...
func RRSchedule(w io.Writer, title string, processes []Process) {
	...
	for len(queue) > 0 {
		...
		if proc.BurstDuration > quantum {
			proc.BurstDuration -= quantum
			queue = append(queue, proc)
			serviceTime += quantum
		} else {
			serviceTime += proc.BurstDuration
			waitingTime = serviceTime - proc.BurstDuration - proc.ArrivalTime
			totalWait += float64(waitingTime)

			turnaround := waitingTime + proc.BurstDuration
			totalTurnaround += float64(turnaround)

			completion := serviceTime
			lastCompletion = float64(completion)

			schedule[proc.ProcessID-1] = []string{
			...
			gantt = append(gantt, TimeSlice{
				PID:   proc.ProcessID,
				Start: serviceTime - proc.BurstDuration,
				Stop:  serviceTime,
			})
		}
	}
